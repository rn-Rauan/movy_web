...
function ScreenViagens({ t }) {
  const trips = [
    { date: '21/05', time: '20:00', from: 'Campo Maior', to: 'Piripiri', booked: 0, total: 30, status: 'Cancelada', driver: 'Akinuku' },
    { date: '22/05', time: '20:00', from: 'Campo Maior', to: 'Piripiri', booked: 0, total: 30, status: 'Agendada', driver: 'Akinuku' },
    { date: '25/05', time: '20:00', from: 'Campo Maior', to: 'Piripiri', booked: 8, total: 30, status: 'Agendada', driver: 'Akinuku' },
    { date: '26/05', time: '20:00', from: 'Campo Maior', to: 'Piripiri', booked: 14, total: 30, status: 'Confirmada', driver: 'Akinuku' },
    { date: '27/05', time: '20:00', from: 'Campo Maior', to: 'Piripiri', booked: 2, total: 30, status: 'Agendada', driver: 'Akinuku' },
  ];
  ...
  return (
    <Phone t={t} label="02 Viagens">
      <TopBar title="Viagens" t={t} back action={
        <button style={{
      ...
      <Scroll>
        <div style={{ padding: '12px 16px 24px' }}>
          ...
          <div style={{ position: 'relative', marginBottom: 12 }}>
            <I.search size={16} color={t.muted} style={{ position: 'absolute', left: 14, top: '50%', transform: 'translateY(-50%)' }} />
            <input
          ...
          <div style={{ display: 'flex', gap: 6, overflowX: 'auto', marginBottom: 16, paddingBottom: 4, marginLeft: -16, paddingLeft: 16, marginRight: -16, paddingRight: 16 }}>
            {status.map((s, i) => (
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', marginBottom: 10, padding: '0 2px' }}>
            <h2 style={{ margin: 0, fontSize: 12, color: t.muted, fontWeight: 700, letterSpacing: 0.5, textTransform: 'uppercase' }}>Esta semana · 5</h2>
            <button style={{ border: 0, background: 'transparent', color: t.muted, fontSize: 12, fontWeight: 600, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 4 }}>
              ...
              <Card key={i} t={t} style={{ padding: 14 }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 12 }}>
                  <div style={{ display: 'flex', alignItems: 'baseline', gap: 8 }}>
                    <div style={{ fontSize: 20, fontWeight: 800, color: t.ink, letterSpacing: -0.5, fontFamily: "'JetBrains Mono', monospace" }}>{trip.date}</div>
                ...
                <div style={{
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: 10, flex: 1, maxWidth: 180 }}>
                    <div style={{ flex: 1 }}>
                      <Bar value={trip.booked} max={trip.total} t={t}
                        color={trip.status === 'Cancelada' ? t.muted : (trip.booked > trip.total * 0.5 ? t.success : t.accent)} />
                    </div>
                    <div style={{ fontSize: 12, fontWeight: 700, fontFamily: "'JetBrains Mono', monospace", color: t.ink }}>
