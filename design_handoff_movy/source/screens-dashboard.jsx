...
// ============================================================================
// SCREEN — Relatório (dashboard detalhado) · Variação A "Financeiro em foco"
// ============================================================================
...
function ScreenDashboardReport({ t, variant = "finance" }) {
  ...
  const month = {
    label: "Maio 2026",
    revTotal: 5840, // confirmada + pendente
    revConfirmed: 4120,
    revPending: 1280,
    revLost: 440, // cancelada
    deltaPct: 18.2, // vs mês anterior
    prevRev: 4940,
    weeks: [820, 1140, 1680, 2200], // R$ por semana
  ...
  const totalTrips =
    month.trips.realized + month.trips.confirmed + month.trips.scheduled + month.trips.canceled;
  ...
  return (
    <Phone t={t} label={`Relatório · ${variant === "finance" ? "Financeiro" : "Operação"}`}>
      ...
      <Scroll>
        <div
          style={{ padding: "14px 16px 28px", display: "flex", flexDirection: "column", gap: 12 }}
        >
          {/* ===== HERO: Receita do mês ===== */}
          <div
            style={{
            ...
            >
              <div style={{ minWidth: 0, flex: 1 }}>
                ...
                <div style={{ display: "flex", alignItems: "baseline", gap: 4, marginTop: 4 }}>
                  ...
                  >
                    {fmtInt(month.revTotal)}
                    <span style={{ fontSize: 20, opacity: 0.5 }}>,00</span>
                  </div>
                </div>
                <div
                  style={{
            ...
            <div style={{ marginTop: 16 }}>
              ...
              <div style={{ display: "flex", gap: 12, marginTop: 10, fontSize: 11 }}>
                ...
                <RevLegend
                  ...
                  label="Pendente"
                  value={fmt(month.revPending)}
                />
                <RevLegend
                  dot="rgba(255,255,255,0.18)"
                  label="Perdida"
                  value={fmt(month.revLost)}
                  muted
                />
              </div>
            </div>
          ...
          {/* ===== Receita por semana (gráfico de barras) ===== */}
          ...
          {/* ===== Viagens do mês — breakdown 2x2 ===== */}
          <div>
            <SectionTitle
              t={t}
              title="Viagens do mês"
              sub={`${totalTrips} no total`}
              action="Ver lista →"
            />
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
              ...
              <TripStatCard
                t={t}
                label="Canceladas"
                value={month.trips.canceled}
                sub={`R$ ${fmt(month.revLost)} perdidos`}
                tone="danger"
              />
            </div>
          </div>
          ...
          >
            ...
            <MiniKPI
              t={t}
              label="Ticket médio"
              value={`R$ ${fmt(month.revTotal / month.passengers).split(",")[0]}`}
              icon={I.money}
              divide
            />
          </div>
            ...
            <Card t={t} style={{ padding: 4 }}>
              {month.topRoutes.map((r, i) => (
                <RouteRow
                  ...
                  route={r}
                  rank={i + 1}
                  last={i === month.topRoutes.length - 1}
                  fmt={fmt}
                />
              ))}
            </Card>
            ...
            <Card t={t} style={{ padding: 4 }}>
              {month.upcoming.map((u, i) => (
                <UpcomingRow
                  ...
                  t={t}
                  item={u}
                  last={i === month.upcoming.length - 1}
                  fmt={fmt}
                />
              ))}
            </Card>
...
// ============================================================================
// Variação B — Operação em foco (viagens primeiro, receita secundária)
// ============================================================================
...
function ScreenDashboardReportOps({ t }) {
  const month = {
    ...
    avgOccupation: 62,
    days: [
      // 4 semanas × 7 dias, valor = receita
      [0, 40, 60, 0, 80, 120, 40],
    ...
    <Phone t={t} label="Relatório · Operação">
      ...
      <Scroll>
        ...
        >
          ...
          >
            <div style={{ display: "flex", alignItems: "flex-start", gap: 12, marginBottom: 14 }}>
              <div style={{ flex: 1, paddingRight: 12, borderRight: `1px solid ${t.lineSoft}` }}>
                ...
                <div style={{ display: "flex", alignItems: "baseline", gap: 6, marginTop: 4 }}>
                  ...
                  >
                    {month.trips.realized}
                  </div>
                  <div style={{ fontSize: 13, color: t.muted }}>de {totalTrips}</div>
                </div>
                <div style={{ marginTop: 6, fontSize: 11, color: t.ink2 }}>
                  <span style={{ color: t.success, fontWeight: 800 }}>{month.passengers}</span>{" "}
                  passageiros · ocup.{" "}
                  <span style={{ fontWeight: 800, color: t.ink }}>{month.avgOccupation}%</span>
                </div>
              </div>
            ...
            <div>
              ...
              >
                <StatusSegment
                  t={t}
                  flex={month.trips.realized}
                  color={t.success}
                  label={month.trips.realized}
                  title="Real."
                />
                <StatusSegment
                  t={t}
                  flex={month.trips.confirmed}
                  color={t.info}
                  label={month.trips.confirmed}
                  title="Confir."
                />
                <StatusSegment
                  t={t}
                  flex={month.trips.scheduled}
                  color={t.muted}
                  label={month.trips.scheduled}
                  title="Agend."
                />
                <StatusSegment
                  t={t}
                  flex={month.trips.canceled}
                  color={t.danger}
                  label={month.trips.canceled}
                  title="Cancel."
                  last
                />
              </div>
          ...
          {/* ===== Heatmap — receita por dia, 4 semanas ===== */}
          <Card t={t} style={{ padding: 14 }}>
            <div
              style={{
                display: "flex",
            ...
            >
              <div>
                ...
                >
                  Quintas concentram a maior receita
                </div>
              </div>
              <div
                style={{
            ...
            >
              ...
              {dayLabels.map((d, i) => (
                <div
                  key={i}
                  style={{ fontSize: 10, color: t.muted, fontWeight: 700, textAlign: "center" }}
                >
                  {d}
                </div>
              ))}
              {month.days.map((wk, wi) => (
                <React.Fragment key={wi}>
                  <div style={{ fontSize: 10, color: t.muted, fontWeight: 600, paddingRight: 6 }}>
                    S{wi + 1}
                  </div>
                  {wk.map((v, di) => {
          ...
          {/* ===== Status detalhado — 4 cards mais grandes ===== */}
          <div>
  ...
  return (
    ...
    >
      <div style={{ display: "flex", alignItems: "baseline", gap: 8 }}>
        <h2 style={{ margin: 0, fontSize: 15, fontWeight: 800, color: t.ink, letterSpacing: -0.2 }}>
          {title}
        </h2>
  ...
  return (
    ...
    >
      ...
      <div style={{ fontSize: 12, fontWeight: 800, fontFamily: "'JetBrains Mono', monospace" }}>
        <span style={{ opacity: 0.55, fontWeight: 600 }}>R$ </span>
        {value}
      </div>
    </div>
  );
