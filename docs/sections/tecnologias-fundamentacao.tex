...
requisitos funcionais, que especificam o que o sistema deve fazer,
de requisitos não funcionais, que expressam atributos de
...
são tratados como requisitos do sistema, expostos pela API e
...
diagramas como o de casos de uso (que captura as interações entre
atores e o sistema), o de classes (que descreve entidades e seus
...
modelo entidade-relacionamento (Seção~\ref{sec:modelagem} e seguintes).

\subsubsection{Arquitetura em Camadas, Clean Architecture e DDD}

A organização de um sistema em camadas separa responsabilidades
distintas, como apresentação, aplicação, domínio e infraestrutura, de modo
...
regra de dependência: as regras de negócio centrais não conhecem
...
modelo de domínio no centro do projeto, propondo conceitos como
...
a comunicação cliente-servidor, a ausência de estado de sessão no
...
comum que plataformas SaaS adotem a multi-tenancy (multitenância), em
que uma mesma instância da aplicação atende a diversas organizações
(\textit{tenants}) mantendo o isolamento lógico dos respectivos
...
Autenticação é o processo de comprovar a identidade de um usuário,
ao passo que autorização determina o que esse usuário pode fazer.
...
como o bcrypt, de modo a resistir a ataques de força bruta
\cite{bcrypt}. O controle de acesso baseado em papéis (RBAC) associa
permissões a papéis, e papéis a usuários, simplificando a administração das
autorizações \cite{nistrbac}. As diretrizes do \textit{OWASP}, particularmente
o \textit{Top 10} e o \textit{Application Security Verification Standard}
(ASVS), orientam a prevenção de vulnerabilidades
...
corretude \cite{pressman2016, sommerville2011}. Os testes unitários
