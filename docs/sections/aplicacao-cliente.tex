...
\toprule
\textbf{Camada} & \textbf{Arquivo (exemplo)} & \textbf{Responsabilidade} \\
\midrule
Serviço & \texttt{services/bookings.service.ts} & Expõe operações sobre os \textit{endpoints} (ex.: listar e cancelar inscrições), escondendo o transporte HTTP. \\
\textit{Hook} (caso de uso) & \texttt{features/bookings/hooks/use\-Bookings.ts} & Invoca o serviço, mantém o estado (conteúdo, erro) e deriva o estado de carregamento. \\
Rota (controlador fino) & \texttt{routes/\_protected.my-bookings.tsx} & Instancia o \textit{hook} e decide entre carregando, erro e conteúdo, delegando a renderização aos componentes. \\
\bottomrule
\end{tabularx}
\end{table}
