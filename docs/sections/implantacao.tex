...
A implantação do Movy reflete a separação em camadas descrita na Seção~\ref{sec:arquitetura}: cada artefato é publicado em uma plataforma especializada no seu papel: cliente e API comunicam-se por HTTP/JSON, e a API acessa o banco por meio de uma conexão PostgreSQL sobre SSL. O ambiente local de desenvolvimento --- no qual o \textit{Docker Compose} sobe, em conjunto, o PostgreSQL e a API --- permanece inalterado e passa a \textbf{coexistir} com o ambiente online, que distribui o sistema em três serviços gerenciados: a aplicação cliente em execução de borda na Cloudflare Workers, a API REST em um \textit{Web Service} no Render e o banco de dados em PostgreSQL gerenciado no Neon.\footnote{Implantação validada nas URLs públicas \texttt{movy-app.rn-labs.workers.dev} (cliente) e \texttt{movy-api-av8q.onrender.com} (API).} O Código~\ref{cod:implantacao} apresenta a topologia de execução, e a Tabela~\ref{tab:implantacao} resume o papel de cada alvo de implantação.

\begin{lstlisting}[caption={Topologia de implantação do sistema}, label={cod:implantacao}]
Navegador
   |  HTTP/JSON
Cloudflare Worker   (Movy Web - frontend, execucao de borda)
   |  HTTP/JSON
Render Web Service  (Movy API - NestJS, servidor persistente)
   |  TCP/SSL (conexao pooled)
Neon PostgreSQL     (banco gerenciado)
\end{lstlisting}
