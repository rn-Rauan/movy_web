...
A adoção do \textbf{NestJS} \cite{nestjs} foi fundamentada na necessidade de uma arquitetura estruturada, com suporte nativo a padrões como injeção de dependência, módulos, \textit{guards} e interceptadores globais, essenciais para implementar \textit{Clean Architecture} e RBAC de forma consistente. A Tabela~\ref{tab:tecnologias_implementadas} resume a \textit{stack} do servidor.

\input{sections/tecnologias-backend-table}

O \textbf{NestJS} fornece injeção de dependência nativa (essencial para \textit{Clean Architecture} e para o padrão Repositório), módulos estruturados que materializam os contextos delimitados (\textit{bounded contexts}) do DDD, \textit{guards} e interceptadores globais para RBAC e \textit{logging}, e um ecossistema maduro (Swagger, JWT, Passport). A \textbf{autenticação} apoia-se em \textit{tokens} JWT \cite{rfc7519} com Passport.js, em um modelo \textit{stateless} adequado a APIs REST: o \textit{refresh token} permite renovar credenciais sem nova autenticação, e o \textit{payload} do JWT é enriquecido no \textit{login} com todos os dados necessários (\texttt{userId}, \texttt{organizationId}, \texttt{role}, \texttt{isDev}), eliminando consultas ao banco a cada requisição. A \textbf{verificação} automatizada utiliza o \textbf{Jest} \cite{jest}, \textit{framework} padrão da comunidade Node/NestJS, integrado ao TypeScript via \textit{ts-jest}, com suporte a testes unitários, de integração e a relatórios de cobertura. As tecnologias de persistência (PostgreSQL e Prisma) e de infraestrutura (Docker e plataformas de nuvem) são tratadas na Subseção~\ref{sec:tec-infra}.
...
A interface é construída com \textbf{React} \cite{react}, biblioteca declarativa baseada em componentes. A adoção do \textbf{TanStack Router} \cite{tanstackrouter} foi motivada por seu roteamento baseado em arquivos com segurança de tipos de ponta a ponta e, sobretudo, pelo suporte a \textit{layouts} aninhados sem segmento de URL (\textit{pathless layouts}), recurso central para o esquema de controle de acesso por papel descrito adiante; o \textbf{TanStack Start} \cite{tanstackstart} provê a camada \textit{full-stack} sobre o \textit{router}. A estilização emprega o \textbf{Tailwind CSS} \cite{tailwind} (\textit{utility-first}, com \textit{tokens} de design), e os componentes baseiam-se no \textbf{shadcn/ui} \cite{shadcnui}, construído sobre as primitivas acessíveis do Radix UI, que adota o modelo de \textit{ownership} --- os componentes são copiados para o repositório e ficam sob controle direto do projeto ---, com a contrapartida de uma política rígida de não editá-los manualmente, atualizando-os apenas via interface de linha de comando. O \textbf{Zod} \cite{zod} é empregado na validação declarativa dos formulários --- combinada a estado controlado no React (\texttt{useState} com \texttt{safeParse}) --- e na validação dos \textit{search params} de rota, permitindo que o mesmo esquema sirva de validação e de fonte de tipos por inferência, reduzindo duplicação. O empacotamento utiliza o \textbf{Vite} \cite{vite}. Registra-se que, no estágio atual, \textbf{não se adotou uma biblioteca dedicada de estado de servidor}: o gerenciamento desse estado é feito por \textit{hooks} próprios sobre a \textit{Context API} do React --- decisão consciente, com a eventual migração para uma biblioteca como o TanStack Query mantida como evolução possível e retomada nas limitações da Seção~\ref{sec:consideracoes}.

\subsection{Banco de Dados e Infraestrutura}
\label{sec:tec-infra}

A persistência apoia-se no \textbf{PostgreSQL} \cite{postgresql}, SGBD relacional confiável com suporte a ACID e transações complexas, acessado por meio do \textbf{Prisma ORM} \cite{prisma}. O Prisma adiciona tipagem forte (geração automática de tipos TypeScript a partir do \textit{schema}), \textit{migrations} versionadas, \textit{query builder} seguro contra \textit{SQL injection}, suporte a \textit{soft-delete} e operações transacionais, além de \textit{seed scripts} para inicialização de dados. No desenvolvimento local, o \textbf{Docker} \cite{docker} padroniza os ambientes e, via \textit{Docker Compose}, orquestra em conjunto o PostgreSQL e a API.

Na implantação, cada artefato é publicado em uma plataforma especializada: a aplicação cliente em execução de borda na \textbf{Cloudflare Workers} \cite{cloudflareworkers}, a API REST em um \textit{Web Service} no \textbf{Render} \cite{render} e o banco de dados em PostgreSQL gerenciado no \textbf{Neon} \cite{neon}. Essa separação reflete a arquitetura em camadas do sistema e é detalhada na Seção~\ref{subsec:implantacao}.
...
Os atores do sistema correspondem aos papéis previstos no controle de acesso, em precedência crescente de capacidades: o \textbf{Visitante} explora o catálogo público de viagens e os perfis públicos de organizações; o \textbf{Passageiro} (usuário autenticado) reserva e gerencia inscrições e pode candidatar-se a motorista; o \textbf{Motorista} visualiza e opera as viagens que lhe são atribuídas; e o \textbf{Administrador} gerencia rotas, viagens, motoristas, veículos, plano e métricas da sua organização. A Figura~\ref{fig:Diagrama de caso de uso.} apresenta o diagrama de casos de uso, organizado pelos quatro contextos de acesso. Por concisão, o diagrama exibe um subconjunto representativo das funcionalidades; a Tabela~\ref{tab:casos_uso} sintetiza os principais casos de uso, e a relação exaustiva consta na Tabela de Requisitos Funcionais (Tabela~\ref{tab:rf}).
...
Esta seção apresenta a concretização do sistema em três frentes: a \textbf{implementação} dos módulos do servidor e da aplicação cliente, com os temas transversais que os sustentam (Subseção~\ref{sec:implementacao}); a \textbf{implantação} em ambiente de nuvem (Subseção~\ref{subsec:implantacao}); e a \textbf{verificação} por testes (Subseção~\ref{subsec:verificacao}).
...
O projeto alcançou os objetivos estabelecidos, entregando um \textbf{MVP funcional} com os treze módulos planejados (93 \textit{endpoints} REST e 93 casos de uso) e a aplicação cliente que os consome. O sistema demonstra uma arquitetura sólida baseada em \textit{Clean Architecture} e \textit{Domain-Driven Design} no servidor, espelhada por uma arquitetura em camadas no cliente. Entre os resultados de maior relevância de engenharia, destacam-se:
