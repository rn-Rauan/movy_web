...
import { canSelectCnhCategory } from "@/lib/cnh";
...
export function CnhCategoriesField({ value, onChange, error, disabled }: Props) {
  function toggle(cat: CnhCategory, checked: boolean) {
    if (checked) {
      if (value.includes(cat) || !canSelectCnhCategory(value, cat)) return;
    ...
    <div className="space-y-1">
      ...
      <div className="grid grid-cols-5 gap-2">
        {ALL_CATEGORIES.map((cat) => {
          ...
          const itemDisabled = disabled || !canSelectCnhCategory(value, cat);
          return (
            <label
              key={cat}
              ...
              } ${itemDisabled ? "opacity-50" : "cursor-pointer hover:bg-accent"}`}
            >
              <Checkbox
                checked={checked}
      ...
      <p className="text-xs text-muted-foreground">
        Selecione uma categoria. A categoria A pode ser combinada com mais uma (ex.: AB, AC). As
        demais são exclusivas.
      </p>
